...
function one() { //можем обратиться в любой момент
    console.log(1);
}

let two = function() { //Function Expession - не можем обратиться до инициализации
    console.log(2);
} 
...
function ask(question, yes, no) {
    if (confirm(question)) yes() //происходит вызов функции yes, которую передали как значение
...
let isSummer = function(month) {
    ...
    if (typeof month !== 'number') { //проверка на тип через определение типа с помощью typeof
...
let sum1 = (a, b) => { //тело функции не в одну строку, поэтому воспользоваться сокращенной формой записи не можем
    let c = a+b;
